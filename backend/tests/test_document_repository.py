...
def test_save_summary_scopes_by_document_and_user():
    client = FakeClient(data=[{"id": "doc-1"}])
    repository = SupabaseDocumentRepository(client)

    repository.save_summary("doc-1", "user-1", "A one paragraph summary.")

    assert ("update", {"summary": "A one paragraph summary."}) in client.builder.calls
    assert ("eq", "id", "doc-1") in client.builder.calls
    assert ("eq", "user_id", "user-1") in client.builder.calls
    assert ("is", "deleted_at", "null") in client.builder.calls


def test_save_summary_reports_whether_a_row_matched():
    matched = SupabaseDocumentRepository(FakeClient(data=[{"id": "doc-1"}]))
    assert matched.save_summary("doc-1", "user-1", "summary") is True

    unmatched = SupabaseDocumentRepository(FakeClient(data=[]))
    assert unmatched.save_summary("foreign", "user-1", "summary") is False
