...
async def lifespan(app: FastAPI):
    # Best-effort, for the same reason the upload path treats it that way: the
    # PDF preview bucket is an enhancement, embeddings are the core feature. A
    # Storage outage must not stop the API from starting, or every frontend
    # request degrades into an opaque "Network Error".
    try:
        ensure_bucket_exists(supabase)
    except Exception:
        logger.warning('Failed to provision the PDF storage bucket', exc_info=True)
    yield
...
async def ask_question(
    ...
    if body.conversation_id is not None:
        ...
        result = query_document(
            question, documents, conversation=conversation, **query_options
        )
        conversation_repository.add_message(
            body.conversation_id, user_id, 'user', question
        )
        conversation_repository.add_message(
        ...
        if body.answer_style != 'balanced':
            query_options['answer_style'] = body.answer_style
        result = query_document(question, documents, **query_options)
